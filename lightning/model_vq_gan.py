...
class VQ_GAN_PL(pl.LightningModule):
    ...
    def training_step(self, batch: torch.Tensor, batch_idx: int):
        ...
        vq_loss = (
            feature_loss +  # image-features vs decoded-features
            q_loss +  # code-book loss
            disc_factor * lambda_ * gen_loss  # gan-like loss
        )
        ...
        if batch_idx % 50 == 0:
            self._log_generated_images(real=batch, fake=decoded_batch)
    ...
    def configure_optimizers(self):
        ...
        opt_vq = optim.Adam(
            ...
            lr=self.lr, eps=1e-08, betas=self.betas,
        )
        opt_disc = optim.Adam(
...
class CodeBook(nn.Module):
    def __init__(self, latent_dim, num_codebook_vectors, beta):
        ...
        self.embedding.weight.data.uniform_(
            -1.0 / num_codebook_vectors, 1.0 / num_codebook_vectors,
        )
    ...
    def forward(self, z):
        ...
        # Compute codebook loss, taking care about gradient flow
        loss = torch.mean((z_q.detach() - z) ** 2) +\
               torch.mean((z_q - z.detach()) ** 2) * self.beta
        z_q = z + (z - z_q).detach()  # trick to keep gradient flowing through z_q!
        
        # Return z_q with original shape of z, as well as its indices and loss
        z_q = z_q.permute(0, 3, 1, 2)
        return z_q, min_dist_indices, loss  # indices used by mini-GPT
...
class VQGANGPT(nn.Module):
    def __init__(self,
        ...
        self.vqgan = self.load_vqgan(
            ckpt_path, img_channels, latent_dim, num_codebook_vectors, beta, device,
        )
    ...
    def top_k_logits(self, logits, top_k):
        ...
        out[out < v[..., [-1]]] = -float('inf')  # rien compris mais set les pas top-k to a very low value
        return out
