...
from models.dc_mae import ViTMAEForPreTraining
from dicom_dataset import get_splitted_dataloaders
from utils import save_pt_img, get_nth_boxed_visualisation
...
def main():
    ...
    # g_optimizer = torch.optim.Adam(model.parameters(), lr=base_lr, weight_decay=weight_decay)
    g_optimizer = torch.optim.Adam(model.parameters(), lr=base_lr)
    ...
    image_ref = next(iter(train_dl))["image"]
    ...
    for epoch in range(epochs):
        ...
        if run_name is not None:
            ...
            with torch.no_grad():
                res = model(image_ref, noise=noise_ref)
                for ex in range(5):
                    save_pt_img(
                        get_nth_boxed_visualisation(model.config.image_size, model.config.patch_size, res, ex), 
